...
fn render_collapsed(appearance: &Appearance, spec: ToggleCardSpec) -> Box<dyn Element> {
    ...
    let card_mouse_state = spec.card_mouse_state;
    let mut on_expand = spec.on_expand;

    Hoverable::new(card_mouse_state, move |_| {
        let title_el = FormattedTextElement::from_str(&title, ui_font_family, 16.)
...
fn render_expanded(appearance: &Appearance, spec: ToggleCardSpec) -> Box<dyn Element> {
    ...
    let title_el = FormattedTextElement::from_str(Box::leak(spec.title.into_boxed_str()), ui_font_family, 16.)
        .with_color(text_color)
        .with_weight(Weight::Normal)
        .with_alignment(TextAlignment::Left)
        .with_line_height_ratio(1.0)
...
pub(super) fn render_inline_segmented_control(
    ...
    let left = build_option(left_label.to_string(), is_left_selected, enabled_mouse, on_left);
    let right = build_option(right_label.to_string(), !is_left_selected, disabled_mouse, on_right);
