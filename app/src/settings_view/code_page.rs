...
impl CodeSubpage {
    ...
    pub fn title(&self) -> String {
        match self {
            Self::Indexing => tr!("settings-code-codebase-indexing"),
            Self::EditorAndCodeReview => tr!("settings-code-editor-and-code-review"),
        }
    }
}
...
impl CodeSettingsPageView {
    pub fn new(ctx: &mut ViewContext<CodeSettingsPageView>) -> Self {
        ...
        let manual_add_directory_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(tr!("settings-code-index-new-folder"), SecondaryTheme)
        ...
        let page = if FeatureFlag::OpenWarpNewSettingsModes.is_enabled() {
            ...
            let categories = vec![
                Category::new(Box::leak(tr!("settings-code-codebase-indexing").into_boxed_str()), codebase_indexing_widgets),
                Category::new(Box::leak(tr!("settings-code-editor-and-code-review").into_boxed_str()), code_editor_review_widgets),
            ];
...
impl SettingsWidget for CodePageWidget {
    ...
    fn render(
        ...
        content.add_child(self.render_settings_subtext(
            global_ai_enabled,
            tr!("settings-code-codebase-index-description"),
            appearance,
        ));
        content.add_child(self.render_settings_subtext(
            global_ai_enabled,
            tr!("settings-code-warp-indexing-ignore-description"),
            appearance,
        ));
...
impl CodePageWidget {
    fn render_autoindexing_rows(
        ...
        let mut rows = vec![
            self.render_autoindex_row(
                tr!("settings-code-auto-index-feature-name"),
            ...
            self.render_settings_subtext(
                ...
                tr!("settings-code-auto-index-description"),
                appearance,
            ),
        ];
        ...
        if codebase_indexing_enabled && !CodebaseIndexManager::as_ref(app).can_create_new_indices()
        {
            rows.push(self.render_settings_subtext(
                false,
                tr!("settings-code-codebase-index-limit-reached"),
                appearance,
            ));
        }
    ...
    fn render_initialization_settings_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        Container::new(
            ui_builder
                .span(tr!("settings-code-initialization-settings"))
                .with_style(UiComponentStyles {
                    font_size: Some(18.0),
                    font_weight: Some(Weight::Semibold),
                    font_color: Some(theme.active_ui_text_color().into()),
    ...
    fn render_codebase_indexing_toggle_row(
        ...
        let label = ui_builder
            .span(tr!("settings-code-codebase-indexing"))
            .with_style(UiComponentStyles {
                font_size: Some(16.0),
                font_weight: Some(Weight::Semibold),
                font_color: Some(theme.active_ui_text_color().into()),
        ...
        let disabled_tooltip_text: Option<String> = match admin_setting {
            AdminEnablementSetting::Enable => Some(tr!("settings-code-indexing-workspace-enabled-admin")),
            AdminEnablementSetting::Disable => Some(tr!("settings-code-indexing-disabled-admin")),
            AdminEnablementSetting::RespectUserSetting if !global_ai_enabled => {
                Some(tr!("settings-code-indexing-disabled-global-ai"))
            }
        ...
        content.add_child(
            Container::new(
                Flex::row()
                    ...
                    .with_child(
                        ui_builder
                            .span(tr!("settings-code-initialized-indexed-folders"))
                            .with_style(UiComponentStyles {
                                font_size: Some(16.0),
                                font_weight: Some(Weight::Semibold),
                                font_color: Some(theme.active_ui_text_color().into()),
        ...
        if !rendered_folder {
            content.add_child(
                Container::new(
                    appearance
                        .ui_builder()
                        .paragraph(tr!("settings-code-no-folders-initialized"))
                        .build()
                        .finish(),
                )
            ...
            let open_rules_button = ui_builder
                ...
                .with_text_and_icon_label(
                    warpui::ui_components::button::TextAndIcon::new(
                        ...
                        tr!("settings-code-open-project-rules"),
    ...
    fn local_indexing_status_presentation(
        ...
        let Some(index_state) = index_state else {
            return IndexingStatusPresentation {
                text: Cow::from(tr!("settings-code-no-index-created")),
        ...
        if index_state.has_pending() {
            let text = match index_state.sync_progress() {
                Some(SyncProgress::Discovering { total_nodes }) => {
                    Cow::from(tr!("settings-code-discovered-chunks", total_nodes = total_nodes))
                }
                Some(SyncProgress::Syncing {
                    completed_nodes,
                    total_nodes,
                }) => Cow::from(tr!("settings-code-syncing-progress", completed_nodes = completed_nodes, total_nodes = total_nodes)),
                None => Cow::from(tr!("settings-code-syncing")),
            };
        ...
        if let Some(completed_successfully) = index_state.last_sync_successful() {
            let (text, color, icon) = if completed_successfully {
                (tr!("settings-code-synced"), theme.ansi_fg_green(), Icon::Check)
            ...
            {
                (
                    tr!("settings-code-codebase-too-large"),
                    theme.ui_warning_color(),
                    Icon::AlertTriangle,
                )
            } else if index_state.has_synced_version() {
                (
                    tr!("settings-code-stale"),
                    theme.nonactive_ui_detail().into_solid(),
                    Icon::ClockRefresh,
                )
            } else {
                (tr!("settings-code-failed"), theme.ui_error_color(), Icon::AlertTriangle)
            };
        ...
        IndexingStatusPresentation {
            text: Cow::from(tr!("settings-code-no-index-built")),
    ...
    fn remote_indexing_status_presentation(
        ...
        match status.state {
            ...
            RemoteCodebaseIndexState::Unavailable => {
                ...
                IndexingStatusPresentation {
                    text: Cow::from(if limit_reached {
                        tr!("settings-code-index-limit-reached")
                    } else {
                        tr!("settings-code-unavailable")
                    }),
            ...
            RemoteCodebaseIndexState::Disabled => IndexingStatusPresentation {
                text: Cow::from(tr!("settings-code-disabled")),
                color: theme.disabled_ui_text_color().into_solid(),
                icon: Some(Icon::SlashCircle),
                refresh_action: Some(IndexingRefreshAction::RequestRemote),
                show_delete: true,
            ...
            RemoteCodebaseIndexState::Queued => IndexingStatusPresentation {
                text: Cow::from(tr!("settings-code-queued")),
                color: theme.disabled_ui_text_color().into_solid(),
                icon: None,
                refresh_action: None,
                show_delete: true,
            ...
            RemoteCodebaseIndexState::Indexing => {
                let text = match (status.progress_completed, status.progress_total) {
                    (Some(completed), Some(total)) => {
                        Cow::from(tr!("settings-code-indexing-progress", completed = completed, total = total))
                    }
                    (Some(completed), None) => Cow::from(tr!("settings-code-indexing-progress-completed", completed = completed)),
                    (None, Some(total)) => Cow::from(tr!("settings-code-indexing-progress-total", total = total)),
                    (None, None) => Cow::from(tr!("settings-code-indexing-dots")),
                };
            ...
            RemoteCodebaseIndexState::Ready => IndexingStatusPresentation {
                text: Cow::from(tr!("settings-code-synced")),
            ...
            RemoteCodebaseIndexState::Stale => IndexingStatusPresentation {
                text: Cow::from(tr!("settings-code-stale")),
            ...
            RemoteCodebaseIndexState::Failed => IndexingStatusPresentation {
                text: Cow::from(tr!("settings-code-failed")),
    ...
    fn render_lsp_servers_subsection(
        ...
        content.add_child(
            ui_builder
                .span(tr!("settings-code-lsp-servers"))
                .with_style(UiComponentStyles {
                    font_size: Some(11.0),
                    font_weight: Some(Weight::Semibold),
                    font_color: Some(theme.disabled_ui_text_color().into()),
    ...
    fn render_suggested_lsp_server_row(
        ...
        let (description, is_installing) = match &repo_status {
            Some(LspRepoStatus::DisabledAndInstalled { .. }) => (tr!("settings-code-installed"), false),
            Some(LspRepoStatus::Installing { .. }) => (tr!("settings-code-installing"), true),
            Some(LspRepoStatus::CheckingForInstallation) => (tr!("settings-code-checking"), true),
            _ => (tr!("settings-code-available-for-download"), false),
        };
        ...
        if is_failed {
            if let Some(server_handle) = server_model.cloned() {
                ...
                let restart_button = ui_builder
                    ...
                    .with_text_label(tr!("settings-code-restart-server"))
                    .build()
                    .with_cursor(Cursor::PointingHand)
                    .on_click(move |ctx, _, _| {
        ...
        #[cfg(not(target_family = "wasm"))]
        {
            ...
            if has_logs {
                ...
                let view_logs_button = ui_builder
                    ...
                    .with_text_label(tr!("settings-code-view-logs"))
                    .build()
                    .with_cursor(Cursor::PointingHand)
                    .on_click(move |ctx, _, _| {
                ...
                match server.state() {
                    LspState::Available { .. } if !server.has_pending_tasks() => (
                        AnsiColorIdentifier::Green
                            ...
                            .into(),
                        tr!("settings-code-available"),
                    ),
                    LspState::Starting | LspState::Available { .. } => (
                        AnsiColorIdentifier::Yellow
                            .to_ansi_color(&theme.terminal_colors().normal)
                            .into(),
                        tr!("settings-code-busy"),
                    ),
                    LspState::Failed { .. } => (
                        AnsiColorIdentifier::Red
                            .to_ansi_color(&theme.terminal_colors().normal)
                            .into(),
                        tr!("settings-code-failed"),
                    ),
                    LspState::Stopped { .. } | LspState::Stopping { .. } => {
                        (theme.disabled_ui_text_color().into_solid(), tr!("settings-code-stopped"))
                    }
                }
            }
            None => (theme.disabled_ui_text_color().into_solid(), tr!("settings-code-not-running")),
        }
    }
}
...
impl SettingsWidget for CodebaseIndexingCategorizedWidget {
    ...
    fn render(
        ...
        content.add_child(render_body_item::<CodeSettingsPageAction>(
            tr!("settings-code-codebase-indexing").into(),
            None,
            LocalOnlyIconState::Hidden,
            ToggleState::Enabled,
            appearance,
            toggle_element,
            Some(tr!("settings-code-codebase-index-description").into()),
        ));
        ...
        if global_ai_enabled && codebase_context_enabled {
            ...
            content.add_child(render_body_item::<CodeSettingsPageAction>(
                tr!("settings-code-auto-index-feature-name").into(),
                None,
                LocalOnlyIconState::Hidden,
                ToggleState::Enabled,
                appearance,
                ui_builder
                    ...
                    .finish(),
                Some(tr!("settings-code-auto-index-description").into()),
            ));

            if !CodebaseIndexManager::as_ref(app).can_create_new_indices() {
                content.add_child(
                    ui_builder
                        .paragraph(tr!("settings-code-codebase-index-limit-reached"))
                        .with_style(UiComponentStyles {
...
impl SettingsWidget for AutoOpenCodeReviewPaneCodeWidget {
    ...
    fn render(
        ...
        render_body_item::<CodeSettingsPageAction>(
            tr!("settings-code-auto-open-code-review").into(),
            None,
            LocalOnlyIconState::Hidden,
            ToggleState::Enabled,
            appearance,
            appearance
                ...
                .finish(),
            Some(tr!("settings-code-auto-open-code-review-description").into()),
        )
    }
}
...
impl SettingsWidget for CodeReviewPanelToggleWidget {
    ...
    fn render(
        ...
        render_body_item::<CodeSettingsPageAction>(
            tr!("settings-code-show-code-review-button").into(),
            None,
            LocalOnlyIconState::Hidden,
            ToggleState::Enabled,
            appearance,
            appearance
                ...
                .finish(),
            Some(
                tr!("settings-code-show-code-review-button-description").into(),
            ),
        )
    }
}
...
impl SettingsWidget for CodeReviewDiffStatsToggleWidget {
    ...
    fn render(
        ...
        render_body_item::<CodeSettingsPageAction>(
            tr!("settings-code-show-diff-stats").into(),
            None,
            LocalOnlyIconState::Hidden,
            ToggleState::Enabled,
            appearance,
            appearance
                ...
                .finish(),
            Some(tr!("settings-code-show-diff-stats-description").into()),
        )
    }
}
...
impl SettingsWidget for ProjectExplorerToggleWidget {
    ...
    fn render(
        ...
        render_body_item::<CodeSettingsPageAction>(
            tr!("settings-code-project-explorer").into(),
            None,
            LocalOnlyIconState::Hidden,
            ToggleState::Enabled,
            appearance,
            appearance
                ...
                .finish(),
            Some(
                tr!("settings-code-project-explorer-description").into(),
            ),
        )
    }
}
...
impl SettingsWidget for GlobalSearchToggleWidget {
    ...
    fn render(
        ...
        render_body_item::<CodeSettingsPageAction>(
            tr!("settings-code-global-file-search").into(),
            None,
            LocalOnlyIconState::Hidden,
            ToggleState::Enabled,
            appearance,
            appearance
                ...
                .finish(),
            Some(tr!("settings-code-global-file-search-description").into()),
        )
    }
}
