...
// Auth token placeholder text is now resolved via tr!() at render time.
...
impl AuthViewBody {
        ...
        let auth_token_input = ctx.add_typed_action_view(|ctx| {
            ...
            let placeholder_text =
                if matches!(experiment_group, Some(AuthFlowInstructions::Experiment)) {
                    tr!("auth-token-placeholder-experimental")
                } else {
                    tr!("auth-token-placeholder")
                };
            ...
            Flex::column()
                ...
                .with_child(
                    Flex::row()
                        .with_child(
                            ui_builder
                                .paragraph(&tr!("auth-adjust-privacy"))
                                .with_style(disclaimer_styles)
                                .build()
                                .finish(),
                        )
    ...
    fn render_sign_up_button(
        ...
        ui_builder
            ...
            .with_centered_text_label(tr!("auth-sign-up"))
            .build()
    ...
    fn render_sign_up_later_row(&self, ui_builder: &UiBuilder) -> Box<dyn Element> {
        Container::new(
            Flex::row()
                .with_child(
                    ui_builder
                        .span(&tr!("auth-dont-want-sign-in"))
                        .build()
                        .finish(),
                )
                .with_child(
    ...
    fn render_sign_in_later_confirm_row(&self, ui_builder: &UiBuilder) -> Box<dyn Element> {
        Container::new(
            Flex::column()
                .with_child(
                    ui_builder
                        .paragraph(&tr!("auth-skip-confirm-title"))
                        .build()
                        .finish(),
                )
                .with_child(
                    ui_builder
                        .paragraph(&tr!("auth-skip-confirm-desc-1"))
                        .build()
                        .finish(),
                )
                .with_child(
                    Flex::row()
                        .with_child(
                            ui_builder
                                .span(&tr!("auth-skip-confirm-desc-2"))
                                .build()
                                .finish(),
                        )
                        .with_child(
    ...
    fn render_force_login_disclaimer(
        ...
        let text = match self.variant {
            AuthViewVariant::RequireLoginCloseable  => {
                &tr!("auth-force-login-ai")
            }
            AuthViewVariant::HitDriveObjectLimitCloseable => {
                &tr!("auth-force-login-drive")
            }
            AuthViewVariant::ShareRequirementCloseable => {
                &tr!("auth-force-login-share")
            }
            _ => "",
        };
    ...
    fn render_header(&self, appearance: &Appearance, ui_builder: &UiBuilder) -> Box<dyn Element> {
        ...
        let text = match self.variant {
            AuthViewVariant::Initial => &tr!("auth-welcome-header"),
            AuthViewVariant::RequireLoginCloseable
            | AuthViewVariant::HitDriveObjectLimitCloseable
            | AuthViewVariant::ShareRequirementCloseable => &tr!("auth-signup-header"),
        };
    ...
    fn render_browser_open_content(
        ...
        let header = Container::new(
            ui_builder
                .paragraph(&tr!("auth-browser-open-header"))
                .with_style(header_styles)
                .build()
                .finish(),
        )
